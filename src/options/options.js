...
// === Folder Triggers Management ===

async function loadFolderTriggers() {
    ...
    triggers.forEach((trigger, index) => {
        ...
        row.innerHTML = `
            <td><code>${escapeHtml(trigger.pattern)}</code></td>
            <td><code>${escapeHtml(trigger.folder)}</code></td>
            <td><button type="button" class="btn-remove" data-index="${index}">🗑️</button></td>
        `;
    ...
    list.querySelectorAll('.btn-remove').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            await removeFolderTrigger(index);
            await loadFolderTriggers();
        });
    });
}
...
async function addFolderTrigger() {
    const pattern = document.getElementById('triggerPattern').value.trim();
    const folder = document.getElementById('triggerFolder').value.trim();
    const msgEl = document.getElementById('triggerValidationMessage');
    
    msgEl.classList.add('hidden');
    
    if (!pattern || !folder) {
        msgEl.textContent = 'Pattern and folder are required';
        msgEl.classList.remove('hidden');
        return;
    }
    
    const triggers = await getFolderTriggers();
    triggers.push({ pattern, folder });
    
    await browser.storage.sync.set({ folderTriggers: triggers });
    
    document.getElementById('triggerPattern').value = '';
    document.getElementById('triggerFolder').value = '';
    
    await loadFolderTriggers();
}

async function removeFolderTrigger(index) {
    const triggers = await getFolderTriggers();
    triggers.splice(index, 1);
    await browser.storage.sync.set({ folderTriggers: triggers });
}

// Initialize triggers section
function setupFolderTriggersSection() {
    document.getElementById('addTriggerBtn').addEventListener('click', addFolderTrigger);
    loadFolderTriggers();
}
...
async function loadFolderTriggers() {
    const triggers = await getFolderTriggers();
    const list = document.getElementById('triggersList');
    if (!list) return; // секция может не быть на странице
    
    list.innerHTML = '';
    
    if (triggers.length === 0) {
        const row = document.createElement('tr');
        row.innerHTML = `<td colspan="3" style="text-align: center; color: #90A4AE;">No rules yet</td>`;
        list.appendChild(row);
        return;
    }
    ...
    triggers.forEach((trigger, index) => {
        ...
        row.innerHTML = `
            <td><code>${escapeHtml(trigger.pattern)}</code></td>
            <td><code>${escapeHtml(trigger.folder)}</code></td>
            <td><button type="button" class="btn-remove" data-index="${index}" aria-label="Remove rule">🗑️</button></td>
        `;
        list.appendChild(row);
    });
...
async function addFolderTrigger() {
    ...
    if (document.getElementById('triggerPattern')) document.getElementById('triggerPattern').value = '';
    if (document.getElementById('triggerFolder')) document.getElementById('triggerFolder').value = '';
...
function setupFolderTriggersSection() {
    ...
    if (addBtn) {
        addBtn.addEventListener('click', addFolderTrigger);
    }
    loadFolderTriggers();
}