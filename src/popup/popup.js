...
addEventListener('DOMContentLoaded', async (event) => {
  const pendingUrl = await browser.runtime.sendMessage({ command: 'getInitialUrl' });
  
  // Используем pendingUrl ТОЛЬКО если это валидный абсолютный URL
  let url;
  if (pendingUrl && typeof pendingUrl === 'string' && pendingUrl.indexOf("://") !== -1) {
      url = pendingUrl;
  } else {
  ...
  // 🎯 Авто-подстановка папки по триггеру
  let folderValue = await getDefaultFolder() || "";
  const checkUrl = pendingUrl && pendingUrl.indexOf("://") !== -1 ? pendingUrl : url;
  if (checkUrl) {
      const matchedFolder = await getFolderForUrl(checkUrl);
      if (matchedFolder) {
          folderValue = matchedFolder;
      }
  }
  document.getElementById('folder').value = folderValue;
