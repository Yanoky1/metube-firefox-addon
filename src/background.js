...
let pendingContextMenuUrl = null;
...
async function sendWithLoadingIndicator(url) {
    // Сохраняем только валидные абсолютные URL
    if (url && typeof url === 'string' && url.indexOf("://") !== -1) {
        pendingContextMenuUrl = url;
    } else {
        pendingContextMenuUrl = null;
    }
    browser.browserAction.setPopup({ popup: "popup/popup.html" });
...
browser.runtime.onMessage.addListener(async (message) => {
  ...
  } else if (message.command === "getInitialUrl") {   // ← добавить
    const url = pendingContextMenuUrl;
    pendingContextMenuUrl = null;
